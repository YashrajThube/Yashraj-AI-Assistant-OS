...
from app.core.environment_validation import probe_gemini_configuration, validate_environment
...
from app.services.google_auth_service import get_auth_status, get_oauth_diagnostics
...
async def _run_startup_validation() -> None:
    ...
    if env_validation["issues"]:
        ...
        logger.info(json.dumps({"event": "startup_check", "service": "environment", "status": "ok"}, ensure_ascii=True))
    ...
    oauth_diagnostics = await get_oauth_diagnostics()
    logger.info(
        json.dumps(
            {
                "event": "startup_check",
                "service": "google_oauth_diagnostics",
                "token_exists": oauth_diagnostics["token_exists"],
                "credentials_valid": oauth_diagnostics["credentials_valid"],
                "refresh_token_exists": oauth_diagnostics["refresh_token_exists"],
                "auth_status": oauth_diagnostics["auth_status"],
            },
            ensure_ascii=True,
        )
    )
...
async def health_database(db: AsyncSession = Depends(get_db)):
    try:
        await db.execute(text("SELECT 1"))
        return ok_response(data={"database": "ok"}, message="Database reachable")
    except Exception as exc:
        return error_response(message="Database check failed", status_code=503, data={"database": "error", "detail": str(exc)[:300]})


@app.get("/health/gemini")
async def health_gemini():
    result = probe_gemini_configuration()
    status = 200 if result["status"] == "ok" else 503
    return ok_response(data={"gemini": result}, message=result["message"], status_code=status) if status == 200 else error_response(message=result["message"], status_code=status, data={"gemini": result})
...
async def health_config():
    ...
    return ok_response(data={"configuration": result}, message="Configuration check completed", status_code=status) if status == 200 else error_response(message="Configuration validation failed", status_code=status, data={"configuration": result})

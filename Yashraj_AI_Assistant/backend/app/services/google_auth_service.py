...
def _inspect_oauth_state_sync() -> dict:
    client_secrets_file = _client_secrets_path()
    token_file = _token_path()
    ...
    if client_secrets_file.exists():
        try:
            client_data = json.loads(client_secrets_file.read_text(encoding="utf-8"))
            credentials_valid = "web" in client_data and bool(client_data["web"].get("client_id"))
        except Exception:
            credentials_valid = False
    ...
    if token_file.exists():
        try:
            ...
            refresh_token_exists = bool(token_data.get("refresh_token"))
        except Exception:
            refresh_token_exists = False
...
async def get_oauth_diagnostics() -> dict:
    diagnostics = await run_in_threadpool(_inspect_oauth_state_sync)
    diagnostics["auth_status"] = await get_auth_status()
    logger.info(
        "Google OAuth startup diagnostics",
        extra={
            "token_exists": diagnostics["token_exists"],
            "credentials_valid": diagnostics["credentials_valid"],
            "refresh_token_exists": diagnostics["refresh_token_exists"],
            "auth_status": diagnostics["auth_status"],
            "redirect_uri": diagnostics["redirect_uri"],
